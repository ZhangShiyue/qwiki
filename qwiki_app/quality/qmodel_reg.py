# coding: utf-8
"""
implement quality assessment regression model using theano
"""
import time
import numpy
import theano
...
def build_model(tparams, options):
    ...
    pred = tensor.dot(hidden, tparams['U']) + tparams['b']
    fpred = tensor.flatten(pred)

    f_pred_prob = theano.function([x, mask, x_text], pred, name='f_pred_prob')
    f_pred = theano.function([x, mask, x_text], fpred, name='f_pred')

    cost = tensor.square(fpred - y).mean()
...
def get_quality(
        input,
        reload_model=settings.QMODEL_DIR + "/model_combine_6_64_50_reg.npz",  # Path to a saved model we want to start from.
        dim_input=6,
        dim_proj=64,
        dim_text=11,
        ydim=6,
...
if __name__ == '__main__':
    ...
    input = get_data("Bonfire Night")

    y_pred = get_quality(input, reload_model="../static/model_combine_6_64_50_reg.npz")
    print y_pred
